...
# Provide some high quality book cover urls from unsplash
book_images = [
...
for i, b in enumerate(books_data):
    img = book_images[i % len(book_images)]
    # To add more randomness to image:
    img = random.choice(book_images)
    fantasy_books_json.append({
        "title": b[0],
        "genre": b[1],
        "tag": b[2],
        "price": b[3],
...
# Add unique ids
for idx, b in enumerate(fantasy_books_json):
    b['id'] = idx + 1

# Generate string
js_array_str = "const fantasyBooks = " + json.dumps(fantasy_books_json, indent=4) + ";"

file_path = r"c:\Users\PC\Documents\For-the-finals-2nd-year-main\Finals\index.html"
...
# Replace the array
content = re.sub(
    r"const fantasyBooks = \[\s*\{.*?\}\s*\];",
    js_array_str,
    content,
    flags=re.DOTALL
)

# Advanced feature 1: Update the Genre Filter to include new genres and a sort dropdown
filter_area_regex = r'<select id="genreFilter">.*?</select>'
new_filters = """<select id="genreFilter">
...
content = re.sub(filter_area_regex, new_filters, content, flags=re.DOTALL)

# Advanced feature 2: Add pagination button & 3D Tilt Script
# find the end of the books grid
grid_end_regex = r'<div id="booksContainer" class="books-grid"></div>\s*</div>'
pagination_html = """<div id="booksContainer" class="books-grid"></div>
...
content = re.sub(grid_end_regex, pagination_html, content)

# Modify the script to handle pagination, sorting, and tilt effect
# We will inject logic into `renderBooks()`
...
content = re.sub(r'let cart = \[\];.*?function showToast\(text\) \{', new_script_top.split("function showToast(text) {")[0] + "function showToast(text) {", content, flags=re.DOTALL)
content = re.sub(r'// Render Books.*?function updateTrackerUI', new_script_top.split("// Render Books")[1] + "\n\n    function updateTrackerUI", content, flags=re.DOTALL)
...
# Update number of copies text in hero section
content = content.replace("Over 200 enchanted copies await.", f"Over {len(fantasy_books_json)} unique enchanted copies await.")
